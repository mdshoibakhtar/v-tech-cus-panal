...
import { QuantityCounter } from "../../components/prodetails/Counter";
...
import { useSetCartMutation, useSetWishListMutation } from "../../components/products/productSlice";
import { FacebookIcon, FacebookShareButton, WhatsappIcon, WhatsappShareButton } from "react-share";

function TopRentProd({ param }) {
    ...
    const getAllDeta = async () => {
        ...
        try {
            const user = await axios.get(`https://onlineparttimejobs.in/api/serviceProductRent/productpublic/${param}`)
                ...
                <div className="col-md-6">
                    <div className="_product-detail-content">
                        ...
                        <div className="_p-price-box">
                            ...
                            <div className="_p-add-cart">
                                <div className="_p-qty">
                                    <span style={{ fontSize: '16px' }} >Select tenure </span>
                                    <div className="fortypePro">

                                        <div className="MuiBox-root css-1w2xuzt">
                                            <div className="MuiStack-root css-4298ue">
                                                {data?.rents[data?.rents?.length - 1]?.duration_obj.map((item, i) => {
                                                    return <div key={item._id} onClick={() => { setActiveRent(data?.rents, i) }} className={`MuiGrid-root MuiGrid-container css-1391e6b ${i === countRent && 'activeRent'}`}>
                                                        <div className="MuiGrid-root MuiGrid-item MuiGrid-grid-xs-12 css-86bxfj">
                                                            <p className="MuiTypography-root MuiTypography-body2 css-1lwex0m">rent per {item?.name}</p>
                                                        </div>
                                                        {data?.rents[data?.rents?.length - 1]?.services.map((val) => {
                                                            if (val.duration === item._id) {
                                                                return <div key={val._id} className="MuiGrid-root MuiGrid-item MuiGrid-grid-xs-12 css-p0rhwy">
                                                                    <p className="MuiTypography-root MuiTypography-body2 css-2e30d2">₹ {val?.sale_price}</p>
                                                                </div>
                                                            }
                                                        })}
                                            ...
                                            <p className="MuiTypography-root MuiTypography-body2 css-hq3c65">Refundable Security Deposit: ₹ {data?.rents[data?.rents?.length - 1]?.refundable}</p>
                                            <p className="MuiTypography-root MuiTypography-body2 css-hq3c65">Installable Amount: ₹ {data?.rents[data?.rents?.length - 1]?.installable}</p>
                                        </div>
                                    ...
                                    <h6 className="MuiTypography-root MuiTypography-body1 css-wk6fj0">Rs {data?.rents[data?.rents?.length - 1]?.services[0].sale_price} per-{data?.rents[data?.rents?.length - 1]?.duration_obj[0].name}</h6>
                                </div>
                            </div>
                            <div className="_p-add-cart">
                                <div className="_p-qty">
